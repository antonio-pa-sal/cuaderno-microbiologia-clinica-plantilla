---
title: "P20 — Urocultivo"
practice_id: "P20"
unit: "UD6 — Bacteriología: estudio de muestras biológicas"
learning_outcomes: "RA04 (CE04.a, CE04.b, CE04.c, CE04.e y CE04.f); RA05 (CE05.a, CE05.c–CE05.f y CE05.h)"
transversal_learning_outcome: "RA01"
page_type: "Página de práctica y cuaderno digital del alumnado"
student_completion: "Individual o por equipo, según la organización del grupo"
reference_document: "P17_Urocultivo.pdf"
---

# P20 — Urocultivo

> **Estructura de la página en Google Sites.** Usa el título como encabezado principal. Mantén los contenidos del profesorado visibles y sin edición. Agrupa los bloques rellenables en el orden de trabajo: planificación de la muestra, siembra o análisis, lectura, recuento, interpretación y reflexión. Inserta imágenes solo cuando estén autorizadas, con pie de foto técnico, procedencia y finalidad. No muestres rostros, datos clínicos identificables, códigos sensibles ni muestras reales no autorizadas.

| Clave de lectura | Uso en la página |
| --- | --- |
| **[PROFESOR · FIJO]** | Información que se mantiene visible y no se modifica. |
| **[ALUMNADO · RELLENABLE]** | Campo, tabla o texto que completa el alumnado durante o tras la práctica. |

## 1. Identificación de la práctica

### [PROFESOR · FIJO] Datos curriculares

| Elemento | Información |
| --- | --- |
| Código y título | P20 — Urocultivo |
| Unidad didáctica | UD6 — Bacteriología: estudio de muestras biológicas |
| Resultados de aprendizaje | RA04: preparación, siembra y cultivo; RA05: identificación e interpretación; RA01 transversal |
| Criterios vinculados | RA04: CE04.a, CE04.b, CE04.c, CE04.e y CE04.f; RA05: CE05.a, CE05.c–CE05.f y CE05.h |
| Finalidad | Integrar muestra, cultivo, recuento, identificación e interpretación en un proceso clínico simulado o autorizado. |
| Evidencia principal | Selección del proceso, recuento de viables, lectura de morfologías, interpretación prudente y comunicación del resultado. |
| Instrumentos | Prueba práctica, caso clínico, portfolio/cuaderno y prueba teórica. |
| Relación FEOE | Prepara la transferencia al núcleo RA05; la actividad profesional efectiva solo se activa con plan individual y entidad confirmados. |

### [ALUMNADO · RELLENABLE] Datos de realización

| Campo | Registro del alumnado |
| --- | --- |
| Nombre y apellidos | [Escribe aquí] |
| Grupo / equipo de trabajo | [Escribe aquí] |
| Fecha de realización | [dd/mm/aaaa] |
| Fecha de lectura o análisis | [dd/mm/aaaa] |
| Rol desempeñado | [Preparación / siembra / lectura / recuento / cálculo / registro / análisis] |
| Modalidad de trabajo | [Muestra simulada / control docente / demostración / análisis de datos / muestra real autorizada] |
| Código de muestra, placa o evidencia | [Escribe aquí, sin datos sensibles] |

## 2. Finalidad y resultados esperados

### [PROFESOR · FIJO]

El urocultivo permite relacionar el crecimiento de microorganismos viables con una muestra de orina y estimar su carga mediante unidades formadoras de colonias por mililitro (UFC/mL). El recuento informa sobre células capaces de crecer en las condiciones empleadas; no incluye necesariamente bacterias muertas o no cultivables.

La práctica integra la trazabilidad de la muestra, la siembra con asa calibrada o equivalente, el crecimiento en medios apropiados —por ejemplo, agar sangre y agar CLED—, el recuento, la observación de morfologías coloniales, la identificación posterior y la comunicación de un resultado. El significado clínico depende del tipo de muestra, la forma de recogida, el paciente, la calidad del cultivo, los controles y los criterios vigentes aportados por el profesorado.

Al finalizar, el alumnado deberá ser capaz de:

- describir la modalidad, procedencia y limitaciones de la muestra;
- aplicar o analizar una siembra de orina siguiendo el procedimiento autorizado;
- calcular UFC/mL conservando volumen, dilución, recuento y unidad;
- distinguir morfotipos y valorar crecimiento monomicrobiano, mixto o no interpretable;
- explicar la información cualitativa que aporta un medio diferencial como CLED; y
- comunicar un resultado positivo, negativo, dudoso o contaminado sin convertirlo en diagnóstico o prescripción.

> **Límite de interpretación.** Un recuento aislado no confirma por sí mismo una infección urinaria. La interpretación requiere contexto clínico, calidad de la muestra, criterios profesionales vigentes y, cuando proceda, identificación y sensibilidad realizadas con el circuito autorizado.

### [ALUMNADO · RELLENABLE] Objetivo personal

Completa antes de empezar:

> Mi objetivo técnico en esta práctica es [escribe una acción observable y segura que quieras realizar o comprender].

## 3. Recursos, seguridad y autorización

### [PROFESOR · FIJO] Recursos previstos

| Recurso | Uso previsto |
| --- | --- |
| Muestra simulada, control docente o muestra real autorizada | Fuente de datos para la siembra y el análisis. |
| Asa calibrada de 1 µL o 10 µL, o equivalente validado | Transferencia de un volumen conocido cuando el procedimiento lo autorice. |
| Agar sangre y agar CLED, u otros medios indicados | Recuperación, recuento y observación de características diferenciales. |
| Incubación y material de lectura | Condiciones definidas por el [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf) del centro; el documento de referencia propone 24 h a 37 °C para el ejercicio histórico. |
| EPI, campana o puesto aséptico y residuos | Protección, prevención de exposición y cierre seguro. |
| [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf), controles y criterios vigentes | Referencia para la siembra, lectura, cálculo e interpretación. |

### [PROFESOR · FIJO] Condiciones de seguridad y confidencialidad

- La modalidad por defecto es muestra simulada, control docente, placa preparada o conjunto de datos. No se presupone el uso de orina clínica real.
- Una muestra real solo se utilizará con autorización, anonimización, evaluación de riesgos, supervisión y circuito de bioseguridad del centro.
- No se fotografiarán etiquetas clínicas, nombres, historias, ubicaciones sensibles ni información que permita identificar a una persona.
- La muestra se homogenizará suavemente solo si el [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf) autorizado lo indica; se evitará la formación de espuma y la generación de aerosoles.
- La manipulación, incubación, apertura y eliminación de materiales se realizará únicamente según la instrucción local. El alumnado no abrirá ni reincubará placas fuera de ella.

> **Alternativa segura.** Si no se autoriza la siembra o el cultivo, se trabajará con imágenes, placas cerradas, resultados preparados o paneles simulados. Interpretar un resultado preparado no equivale a haber procesado una muestra clínica.

### [ALUMNADO · RELLENABLE] Comprobación previa

| Verificación | Sí / No / No aplica | Observación o referencia |
| --- | --- | --- |
| Conozco la modalidad autorizada de esta sesión | [ ] | [Escribe aquí] |
| La muestra o el conjunto de datos están codificados y autorizados | [ ] | [Escribe aquí] |
| Dispongo de EPI y del puesto indicado | [ ] | [Escribe aquí] |
| He identificado el volumen del asa y la unidad de cálculo | [ ] | [Escribe aquí] |
| Conozco las condiciones de lectura y la ruta de residuos | [ ] | [Escribe aquí] |

## 4. Fundamento técnico

### [PROFESOR · FIJO]

El recuento de viables se expresa como UFC/mL porque cada colonia visible representa, de forma aproximada, una unidad capaz de originar crecimiento en el medio y las condiciones empleadas. La relación general es:

> **UFC/mL = colonias contadas ÷ volumen inoculado (mL) × factor de dilución**

Cuando se usa un asa calibrada sin dilución adicional, 1 µL equivale a 0,001 mL y 10 µL equivale a 0,01 mL. Por tanto, el alumnado debe conservar siempre el volumen real, el recuento y cualquier factor indicado por el [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf), sin sustituirlos por un multiplicador memorizado.

El agar sangre permite observar crecimiento y rasgos coloniales; el agar CLED ayuda a valorar crecimiento y ciertas diferencias cualitativas del cultivo urinario. Ninguno de estos medios identifica por sí solo una especie ni permite interpretar un resultado sin considerar pureza, número de morfotipos, tipo de muestra y contexto.

Los criterios de significación dependen del tipo de muestra y del paciente. Las tablas históricas del documento de referencia sirven para plantear el ejercicio, pero los puntos de corte y comentarios clínicos deben ser aportados o validados por el profesorado con la referencia vigente aplicable.

### [ALUMNADO · RELLENABLE] Hipótesis de trabajo

Antes de realizar o revisar la actividad, responde:

> ¿Qué patrón esperas encontrar —sin crecimiento, un morfotipo predominante, varios morfotipos o resultado no valorable— y qué dato necesitarás para decidir si el recuento es interpretable?

[Escribe aquí]

## 5. Flujo de trabajo y criterios de calidad

### [PROFESOR · FIJO] Flujo general

1. Confirmar modalidad, autorización, código, [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf), EPI y límites de manipulación.
2. Revisar o homogenizar suavemente la muestra según la instrucción autorizada.
3. Tomar el volumen conocido con asa calibrada o analizar el dato preparado equivalente.
4. Descargar el inóculo y distribuirlo sobre el medio conforme al [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf): trazo inicial, distribución en superficie y agotamiento progresivo.
5. Retirar el material de inoculación y custodiar o incubar el sistema según la instrucción local; el ejercicio histórico propone 24 h a 37 °C.
6. Contar colonias, describir morfotipos y conservar imágenes o datos trazables.
7. Calcular UFC/mL con el volumen y los factores correctos.
8. Comparar con el criterio profesional vigente aplicable, valorar contaminación o errores y comunicar el resultado con sus límites.

### Procedimiento específico

**[PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf) de referencia:** [SEIMC, procedimiento 14a: procesamiento microbiológico de la orina](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf)

1. Confirma muestra simulada o control, código, agar sangre/CLED, asa calibrada, EPI, [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf) y residuos.
2. Homogeneiza suavemente la muestra o revisa el dato preparado; evita espuma, derrames y aerosoles.
3. Toma el volumen conocido con asa calibrada o analiza su equivalente docente; registra microlitros y procedencia.
4. Descarga e inocula siguiendo trazo inicial, distribución superficial y agotamiento indicados.
5. Incuba o recibe placas según el procedimiento; no abras ni reincubes fuera de autorización.
6. Cuenta colonias, separa morfotipos y calcula UFC/mL con volumen, dilución y fórmula trazables.
7. Compara con criterio vigente, valora crecimiento mixto y comunica identificación o confirmación pendiente.

### [PROFESOR · FIJO] Criterios de calidad del registro

| Aspecto | Criterio verificable |
| --- | --- |
| Modalidad y autorización | Se declara si hubo simulación, demostración, análisis de placa/datos o muestra real autorizada. |
| Trazabilidad | Código, origen docente, medio, volumen, fecha, responsable y fuente del resultado quedan relacionados. |
| Siembra o análisis | Se reconoce la distribución del inóculo, el medio utilizado y cualquier desviación. |
| Recuento y cálculo | Se conserva el recuento bruto, el volumen, la dilución, la fórmula y la unidad final. |
| Morfología y pureza | Se registran morfotipos, crecimiento mixto, contaminación o ausencia de crecimiento. |
| Interpretación | Se separan dato, criterio vigente, conclusión presuntiva y limitaciones clínicas. |
| Seguridad y confidencialidad | Se documentan EPI, incidencias, residuos y anonimización. |

---

# Tu cuaderno de prácticas

## 6. Planificación y datos iniciales

### [ALUMNADO · RELLENABLE] Ficha de trabajo

| Elemento | Registro del alumnado |
| --- | --- |
| Pregunta o caso clínico simulado | [Escribe aquí] |
| Modalidad asignada | [Muestra simulada / control / demostración / análisis de datos / muestra real autorizada] |
| Tipo de muestra y código | [Escribe aquí, sin datos identificables] |
| Medio o medios utilizados | [Agar sangre / CLED / otro: escribe aquí] |
| Volumen del asa o volumen inoculado | [1 µL / 10 µL / otro: escribe aquí] |
| Dilución o factor de conversión | [Escribe aquí] |
| Condiciones de incubación o fuente del dato | [Escribe aquí] |
| Criterio o referencia vigente que se consultará | [Organismo, documento, versión y fecha] |

### [ALUMNADO · RELLENABLE] Hipótesis inicial

| Hipótesis | Evidencia que la apoyaría | Evidencia que la debilitaría |
| --- | --- | --- |
| Sin crecimiento o recuento no significativo | [Escribe aquí] | [Escribe aquí] |
| Crecimiento predominante de un morfotipo | [Escribe aquí] | [Escribe aquí] |
| Crecimiento mixto o posible contaminación | [Escribe aquí] | [Escribe aquí] |
| Otra hipótesis | [Escribe aquí] | [Escribe aquí] |

### [ALUMNADO · RELLENABLE] Decisión de seguridad

> Indica la medida de seguridad y confidencialidad más importante para esta actividad y explica por qué.

[Escribe aquí]

## 7. Registro del desarrollo

### [ALUMNADO · RELLENABLE]

Registra lo realizado, observado o analizado. Describe decisiones y evidencias sin reproducir ni inventar un protocolo distinto del autorizado.

| Momento | Qué se realizó, observó o analizó | Comprobación de calidad o seguridad | Evidencia asociada |
| --- | --- | --- | --- |
| Identificación de muestra y medio | [Escribe aquí] | [Código, integridad, autorización] | [Foto 1, tabla u otra] |
| Homogeneización o revisión del dato inicial | [Escribe aquí] | [Sin espuma / fuente del dato / supervisión] | [Escribe aquí] |
| Toma con asa, demostración o simulación | [Escribe aquí] | [Volumen, técnica autorizada] | [Foto 2, nota u otra] |
| Descarga y distribución del inóculo | [Escribe aquí] | [Trazo inicial, superficie y agotamiento] | [Escribe aquí] |
| Incubación o recepción de placa/datos | [Escribe aquí] | Condición y custodia según [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf) | [Escribe aquí] |
| Lectura, recuento y morfologías | [Escribe aquí] | [Unidad, pureza, controles] | [Foto 3, tabla u otra] |
| Cálculo e interpretación inicial | [Escribe aquí] | [Fórmula y referencia] | [Escribe aquí] |
| Gestión final de residuos y puesto | [Escribe aquí] | [Ruta indicada] | [Escribe aquí] |

## 8. Resultados y controles

### [ALUMNADO · RELLENABLE] Comprobación de controles

| Elemento que se revisa | Resultado | Evidencia | Consecuencia para la interpretación |
| --- | --- | --- | --- |
| Modalidad, código y procedencia correctamente declarados | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Medio íntegro y correctamente identificado | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Volumen del asa o del inóculo conocido | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Condiciones de incubación o fuente del dato disponibles | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Control, comparación o criterio vigente disponible | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Ausencia de datos identificables y cierre seguro | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |

### [ALUMNADO · RELLENABLE] Recuento y morfología

| Medio | Código | Colonias totales | Morfotipos observados | Colonias por morfotipo | Observaciones de crecimiento |
| --- | --- | ---: | --- | --- | --- |
| Agar sangre | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] |
| Agar CLED | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] |
| Otro/control | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] |

### [ALUMNADO · RELLENABLE] Cálculo de UFC/mL

| Medio o morfotipo | Colonias contadas | Volumen inoculado (mL) | Factor de dilución | Operación | Resultado (UFC/mL) |
| --- | ---: | ---: | ---: | --- | ---: |
| [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] |
| [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] |

> Si no puede calcularse un valor normalizado porque falta el volumen, la dilución o la regla del método, conserva el dato bruto y declara el resultado como no calculable.

### [ALUMNADO · RELLENABLE] Interpretación del medio CLED

¿Qué información cualitativa aporta el CLED en este caso —crecimiento, morfología, fermentación u otra característica indicada por el [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf)— y qué no permite concluir por sí solo?

[Escribe aquí]

### [PROFESOR · FIJO] Tabla de referencia para la actividad

El profesorado debe insertar la tabla o el criterio vigente aplicable al tipo de muestra y al contexto del caso. Como mínimo, debe indicar tipo de muestra, población o situación relevante, umbral o regla, condiciones de validez, interpretación de crecimiento mixto y fuente/fecha.

| Tipo de muestra o situación | Criterio/umbral aportado por el profesor | Condiciones de validez | Interpretación docente |
| --- | --- | --- | --- |
| [Completar por el profesor] | [Completar por el profesor] | [Completar por el profesor] | [Completar por el profesor] |

### [ALUMNADO · RELLENABLE] Valoración del resultado

| Aspecto | Registro |
| --- | --- |
| ¿El recuento es interpretable? | [Sí / no / dudoso; justifica] |
| ¿Cuántos morfotipos aparecen en agar sangre? | [Escribe aquí] |
| ¿El crecimiento parece predominante, mixto, contaminante o ausente? | [Escribe aquí] |
| Comparación con el criterio vigente | [Escribe aquí] |
| Identificación o prueba posterior necesaria | [Escribe aquí] |
| Resultado que se comunicaría | [Negativo / positivo presuntivo / mixto-contaminado / no interpretable / otro] |

### [ALUMNADO · RELLENABLE] Preguntas técnicas del caso

1. Calcula el número de UFC/mL en el agar sangre y valora el resultado según el criterio proporcionado por el profesorado.

   [Respuesta y operación]

2. En función de la morfología de las colonias del agar sangre, ¿cuántos morfotipos o posibles especies se distinguen? Explica por qué la morfología no confirma por sí sola la especie.

   [Respuesta]

3. ¿Qué información cualitativa puede obtenerse del agar CLED en este caso?

   [Respuesta]

4. ¿Cuál sería el procedimiento profesional siguiente ante un resultado positivo o relevante al recuento? Incluye confirmación, identificación, sensibilidad y comunicación, sin prescribir tratamiento.

   [Respuesta]

## 9. Evidencias visuales

### [ALUMNADO · RELLENABLE]

Incluye imágenes propias únicamente si su captura está autorizada. Si proceden del profesorado, de una simulación o de material de referencia, indícalo expresamente. Evita fotografiar etiquetas, datos clínicos, rostros, ubicaciones sensibles o cultivos no autorizados.

### Imagen 1 — Muestra, código y medios

**Inserta aquí una imagen.** Puede mostrar el puesto preparado, medios sin datos sensibles o el esquema de codificación.

- Pie de foto técnico: [Escribe aquí]
- Autoría y procedencia: [Propia / docente / simulación / material de referencia]
- Qué demuestra: [Escribe aquí]

### Imagen 2 — Siembra o distribución del inóculo

**Inserta aquí una imagen.** Puede mostrar una demostración autorizada, una simulación o una representación del trazo inicial y el agotamiento.

- Pie de foto técnico: [Escribe aquí]
- Autoría y procedencia: [Propia / docente / simulación / material de referencia]
- Qué demuestra: [Escribe aquí]

### Imagen 3 — Lectura, colonias y resultado final

**Inserta aquí una imagen.** Puede mostrar una placa cerrada autorizada, la lectura de morfotipos, el recuento o la tabla de cálculo.

- Pie de foto técnico: [Escribe aquí]
- Autoría y procedencia: [Propia / docente / simulación / material de referencia]
- Qué demuestra y qué no permite concluir: [Escribe aquí]

## 10. Incidencias y acciones correctoras

### [ALUMNADO · RELLENABLE]

| Incidencia, duda o desviación | Riesgo o efecto posible | Acción adoptada | Comunicación al profesorado | Impacto en el resultado |
| --- | --- | --- | --- | --- |
| [Escribe aquí o “No se registraron incidencias”] | [Escribe aquí] | [Escribe aquí] | [Sí / no / no aplica] | [Escribe aquí] |

## 11. Interpretación técnica

### [ALUMNADO · RELLENABLE]

Redacta una interpretación breve y fundamentada:

1. Describe la modalidad, la calidad de la muestra o del dato y la validez de los controles.
2. Explica el cálculo de UFC/mL y conserva la unidad, el volumen y cualquier factor aplicado.
3. Relaciona el número de morfotipos y el aspecto de agar sangre/CLED con una hipótesis de crecimiento predominante, mixto o no interpretable.
4. Compara el resultado con el criterio vigente aportado por el profesorado y señala qué confirmación o identificación sería necesaria.
5. Distingue un resultado docente presuntivo de un diagnóstico clínico y no recomienda tratamientos.

[Escribe aquí]

## 12. Conclusión

### [ALUMNADO · RELLENABLE]

> Redacta una conclusión de 4–6 líneas. Debe diferenciar entre: (a) la modalidad realmente desarrollada; (b) el recuento y la morfología obtenidos; y (c) el resultado que puede comunicarse, sus límites y el siguiente paso profesional.

[Escribe aquí]

## 13. Reflexión profesional

### [ALUMNADO · RELLENABLE]

Responde individualmente a las cuatro preguntas y relaciona tus respuestas con los registros de esta práctica.

1. **Procedimiento.** ¿Qué decisión de la toma de muestra, del volumen del asa o de la distribución del inóculo consideras más crítica para que el recuento de UFC/mL sea fiable? Justifica tu elección.

   [Respuesta]

2. **Interpretación.** ¿Cómo cambiaría tu interpretación si aparecieran varios morfotipos en agar sangre, si el CLED mostrara un patrón discordante o si faltara el volumen exacto del inóculo?

   [Respuesta]

3. **Conclusiones.** ¿Qué diferencia hay entre comunicar un urocultivo positivo presuntivo, un crecimiento mixto/contaminado y un resultado no interpretable? Indica qué puedes afirmar y qué no.

   [Respuesta]

4. **Aprendizaje y transferencia.** ¿Cómo aplicarías el cálculo trazable, el control de la calidad de la muestra y la comunicación prudente a otra muestra biológica o a un caso microbiológico nuevo?

   [Respuesta]

## 14. Trazabilidad y entrega

### [ALUMNADO · RELLENABLE]

| Elemento | Registro |
| --- | --- |
| Código de práctica | P20 |
| Fecha de entrega | [dd/mm/aaaa] |
| Enlace o ubicación de las evidencias | [Escribe aquí] |
| Modalidad declarada | [Escribe aquí] |
| Código y procedencia de muestra/datos | [Escribe aquí] |
| Medio, volumen, dilución y condiciones de lectura | [Escribe aquí] |
| Fórmula, recuento bruto y UFC/mL | [Escribe aquí] |
| Morfotipos, incidencias y controles | [Escribe aquí] |
| Criterio vigente y fuente consultada | [Escribe aquí] |
| Ruta de residuos o cierre de actividad | [Escribe aquí] |
| Autoevaluación | [Logrado / en proceso / necesito revisión] |

## 15. Revisión del profesorado

### [PROFESOR · FIJO] Criterios de valoración

| Criterio | Evidencia esperada |
| --- | --- |
| Seguridad, autorización y confidencialidad | Respeta la modalidad, los EPI, la anonimización, el [PNT](https://seimc.org/wp-content/uploads/2025/06/seimc-procedimientomicrobiologia14a.pdf) y el circuito de residuos. |
| Muestra, siembra y trazabilidad | Identifica muestra/dato, medio, volumen, fecha, responsable y fuente de evidencia. |
| Recuento y cálculo | Conserva colonias, morfotipos, volumen, dilución, fórmula, UFC/mL y unidad. |
| Lectura e interpretación | Valora agar sangre/CLED, crecimiento mixto y controles sin sobreinterpretar. |
| Comunicación profesional | Distingue resultado presuntivo, contaminación, no interpretación y pasos posteriores; no prescribe tratamientos. |
| Documentación visual y reflexión | Aporta tres evidencias pertinentes y responde las cuatro preguntas con argumentos profesionales. |

### [PROFESOR · RELLENABLE] Retroalimentación

> [Observaciones del profesorado]
