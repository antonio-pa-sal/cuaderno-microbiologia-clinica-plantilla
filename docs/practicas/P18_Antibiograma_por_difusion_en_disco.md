---
title: "P18 — Antibiograma por difusión en disco"
practice_id: "P18"
unit: "UD4 — Bacteriología: técnicas de identificación y antibiogramas"
learning_outcomes: "RA05 (CE05.f y CE05.h)"
transversal_learning_outcome: "RA01"
page_type: "Página de práctica y cuaderno digital del alumnado"
student_completion: "Individual o por pareja, según la organización del grupo"
reference_document: "P14_Antibiograma disco-placa.pdf"
---

# P18 — Antibiograma por difusión en disco

## 1. Identificación de la práctica

### Datos curriculares

| Elemento | Información |
| --- | --- |
| Código y título | P18 — Antibiograma por difusión en disco |
| Unidad didáctica | UD4 — Bacteriología: técnicas de identificación y antibiogramas |
| Resultado de aprendizaje | RA05: aplicación de técnicas de identificación y valoración de sensibilidad antimicrobiana conforme al procedimiento establecido |
| Criterios vinculados | CE05.f y CE05.h |
| Resultado transversal | RA01: seguridad, higiene, organización, trazabilidad y gestión responsable de materiales y residuos |
| Finalidad | Ejecutar o analizar un estudio de sensibilidad con una cepa docente autorizada e interpretar los halos utilizando el estándar profesional vigente. |
| Evidencia principal | Preparación y trazabilidad, colocación o lectura de discos, medida de halos, interpretación y control de errores. |

### [ALUMNADO · RELLENABLE] Datos de realización

| Campo | Registro del alumnado |
| --- | --- |
| Nombre y apellidos | [Escribe aquí] |
| Grupo / pareja de trabajo | [Escribe aquí] |
| Fecha de realización | [dd/mm/aaaa] |
| Fecha de lectura o revisión | [dd/mm/aaaa] |
| Rol desempeñado | [Preparación / siembra / colocación / lectura / registro / análisis] |
| Modalidad de trabajo | [Ejecución autorizada / demostración docente / simulación / análisis de placa o datos preparados] |
| Código de placa, cepa o evidencia | [Escribe aquí, sin datos sensibles] |

---

## 2. Resumen

En esta práctica realizarás o analizarás un antibiograma por difusión en disco para observar halos de inhibición y clasificarlos según el estándar vigente aplicable. Prepararás o revisarás el inóculo estandarizado, el medio, los discos y los controles, y registrarás la siembra, la incubación prevista y las medidas de cada halo. Comprobarás la calidad de la placa y separarás el dato bruto de su interpretación. El resultado esperado es una tabla trazable de diámetros y categorías de sensibilidad, siempre vinculada al microorganismo, el antimicrobiano y el estándar utilizado. Incluirás imágenes autorizadas, explicarás anomalías y dejarás claro que el ejercicio no prescribe tratamientos ni sustituye una validación clínica.

## 3. Finalidad y resultados esperados


El antibiograma por difusión en disco permite observar la inhibición del crecimiento bacteriano alrededor de discos que contienen antimicrobianos. El principio general es que el antimicrobiano difunde desde el disco en el medio y forma un gradiente de concentración; el diámetro del halo se relaciona con la respuesta de la cepa únicamente cuando se han controlado el inóculo, el medio, los discos, las condiciones de incubación y la lectura.

La medida del halo es un dato bruto. Para transformarlo en una categoría interpretativa se necesita el microorganismo correctamente identificado o asignado al panel docente, el antimicrobiano y su carga, el medio, la metodología y el estándar profesional vigente que corresponda. Los puntos de corte no se fijan en esta página: deben ser aportados y citados por el centro en la fecha de la práctica.

Al finalizar, el alumnado deberá ser capaz de:

- explicar la finalidad de la difusión en disco;
- comprobar la trazabilidad del inóculo, medio, discos y control de calidad;
- obtener o analizar una siembra uniforme y una distribución válida de discos;
- medir y registrar halos en milímetros con sus observaciones y limitaciones; y
- asignar una categoría interpretativa solo mediante el estándar vigente y comunicar resultados dudosos o no válidos.

> **Límite de interpretación.** El antibiograma docente orienta la clasificación de sensibilidad de una cepa frente a determinados antimicrobianos. No prescribe tratamientos, no confirma por sí solo mecanismos de resistencia y no sustituye la validación profesional.

### [ALUMNADO · RELLENABLE] Objetivo personal

Completa antes de empezar:

> Mi objetivo técnico en esta práctica es [escribe una acción observable y segura que quieras realizar o comprender].

---

## 4. Recursos, seguridad y autorización

### Recursos previstos

| Recurso | Uso previsto |
| --- | --- |
| Medio adecuado para difusión en disco | Soporte de la siembra y de la lectura cuando el [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/) lo autorice. |
| Discos antimicrobianos identificados | Panel docente definido por el centro, con carga, lote y vigencia registrados. |
| Inóculo estandarizado | Suspensión docente preparada o analizada conforme a la P15/P16 y al [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/). |
| Cepa docente autorizada, placa preparada o datos de referencia | Material de trabajo según la modalidad. |
| Regla, calibre o sistema de lectura autorizado | Medida del diámetro de los halos y registro del dato bruto. |
| Estándar profesional vigente y controles de calidad | Asignación de categorías y verificación de la validez del ensayo. |
| EPI, desinfección y sistema de residuos | Protección y cierre seguro de la actividad. |

### Condiciones de seguridad

- La ejecución es **condicional**: requiere medio, discos, inóculo, estándar y residuos confirmados.
- Solo se utilizarán cepas docentes, controles, discos y materiales expresamente autorizados.
- La preparación del inóculo y la siembra se realizarán con técnica aséptica, control de aerosoles y supervisión prevista.
- No se abrirán, reincubarán, seleccionarán ni manipularán placas fuera de la instrucción recibida. Las placas con crecimiento se tratarán como material biológico.
- No se interpretará un resultado para orientar una decisión clínica real ni se compartirán datos de muestras o personas.
- Todo residuo biológico, punzante o material contaminado se gestionará por la ruta local indicada.

> **Alternativa segura.** Si no se autoriza el cultivo, la siembra o la lectura directa, se utilizarán placas cerradas preparadas, imágenes, demostración, simulación o datos docentes. Declara la modalidad real: medir un resultado preparado no equivale a haber realizado el antibiograma.

### [ALUMNADO · RELLENABLE] Comprobación previa

| Verificación | Sí / No / No aplica | Observación o referencia |
| --- | --- | --- |
| Conozco la modalidad autorizada de esta sesión | [ ] | [Escribe aquí] |
| La cepa o evidencia está autorizada y trazada | [ ] | [Escribe aquí] |
| El inóculo está estandarizado o su origen está documentado | [ ] | [Escribe aquí] |
| El medio, los discos y el control están identificados | [ ] | [Escribe aquí] |
| Dispongo del estándar vigente y conozco la ruta de residuos | [ ] | [Escribe aquí] |

---

## 5. Fundamento técnico


En la difusión en disco, un crecimiento suficientemente uniforme permite observar zonas de inhibición alrededor de cada disco. El diámetro debe medirse como dato bruto en la unidad indicada por el método, anotando cualquier rasgo que dificulte delimitar el borde: crecimiento dentro del halo, colonias puntiformes, doble zona, swarming, efecto de borde, contaminación o halos que se solapan.

La reproducibilidad depende de múltiples variables: identificación y pureza de la cepa, densidad del inóculo, composición y profundidad del medio, estado y almacenamiento de los discos, distribución, separación, condiciones de incubación y momento de lectura. Una desviación en cualquiera de ellas puede alterar el halo y producir una clasificación errónea.

La categoría profesional —por ejemplo, susceptible, susceptible con exposición aumentada o resistente, según el estándar vigente— no se deduce únicamente del tamaño relativo de un halo. Debe consultarse la tabla actual aplicable al organismo y al antimicrobiano. Si el par organismo-antimicrobiano no está contemplado, el resultado debe quedar como no interpretable o requerir la prueba indicada por el estándar.

El documento histórico de referencia ayuda a reconocer el concepto de Kirby-Bauer y el aspecto de una placa con discos, pero no sustituye el estándar vigente, el [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/) del centro ni las condiciones validadas para la sesión.

### [ALUMNADO · RELLENABLE] Predicción razonada

Antes de realizar o revisar la actividad, responde:

> ¿Qué efecto tendría un inóculo demasiado denso o demasiado escaso sobre los halos? Indica qué observación o control te ayudaría a detectar esa desviación.

[Escribe aquí]

---

## 6. Flujo de trabajo y criterios de calidad

### Flujo general

1. Confirmar la modalidad, la autorización, los EPI, el [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/), el estándar vigente y la ruta de residuos.
2. Identificar cepa o evidencia, inóculo, medio, discos, control y equipo de lectura.
3. Verificar que el inóculo y la superficie de siembra reúnen las condiciones definidas.
4. Realizar la siembra y la colocación de discos según el [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/), observar una demostración, desarrollar una simulación o analizar una placa preparada.
5. Mantener la identificación y custodia de la placa durante la lectura indicada.
6. Registrar el diámetro de cada halo y las observaciones que condicionen la medida.
7. Consultar el estándar profesional vigente y separar medida, categoría y comentario.
8. Documentar controles, incidencias, posibles repeticiones y gestión de residuos.

### Procedimiento específico

**[PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/) de referencia:** [EUCAST, disk diffusion and quality control](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/)

1. Confirma cepa, inóculo estandarizado, Mueller-Hinton, discos, controles, estándar EUCAST y [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/); usa EPI.
2. Prepara o recibe la suspensión y registra método, lote y momento de uso.
3. Inocula uniformemente la superficie y coloca discos identificados según el método; evita solapamientos.
4. Incuba o analiza la placa conforme al procedimiento y custodia indicada; no alteres condiciones.
5. Mide diámetros en milímetros y registra bordes, colonias internas, halos irregulares y unidad.
6. Consulta puntos de corte EUCAST vigentes y clasifica solo con controles y método válidos.
7. Comunica categorías y limitaciones sin prescribir tratamiento; elimina residuos según el circuito.

### Criterios de calidad del registro

| Aspecto | Criterio verificable |
| --- | --- |
| Modalidad | Se declara si hubo ejecución, demostración, simulación o análisis de placa/datos preparados. |
| Trazabilidad | Se identifican cepa/evidencia, inóculo, medio, discos, control, fecha y responsable. |
| Siembra y discos | Se registra si la superficie fue uniforme y si la distribución permite leer los halos sin solapamiento relevante. |
| Medida | Cada diámetro se conserva como dato bruto en milímetros, con método y observaciones. |
| Interpretación | La categoría se asigna solo mediante el estándar vigente aplicable. |
| Control de errores | Se anotan desviaciones, halos irregulares, colonias internas, contaminación o resultados no interpretables. |
| Seguridad | Se documentan técnica aséptica, EPI, incidencias y ruta de residuos. |

---

# Tu cuaderno de prácticas

## 7. Planificación y datos iniciales

### [ALUMNADO · RELLENABLE] Ficha de trabajo

| Elemento | Registro del alumnado |
| --- | --- |
| Pregunta o caso de trabajo | [Escribe aquí] |
| Modalidad asignada | [Ejecución / lectura docente / simulación / análisis de datos] |
| Cepa o material de referencia | [Escribe aquí] |
| Inóculo y método de estandarización | [Escribe aquí] |
| Medio y lote o código | [Escribe aquí] |
| Panel de discos previsto | [Escribe aquí] |
| Control de calidad y respuesta esperada | [Escribe aquí] |
| Estándar profesional vigente | [Organismo, versión y fecha; escribe aquí] |

### [ALUMNADO · RELLENABLE] Decisión de seguridad

> Indica el riesgo principal de trabajar con una placa inoculada y discos antimicrobianos, y la medida concreta que empleaste o que se habría empleado para controlarlo.

[Escribe aquí]

---

## 8. Registro del desarrollo

### [ALUMNADO · RELLENABLE]

Registra lo que realizaste, observaste o analizaste. Describe decisiones y evidencias, sin reproducir ni inventar un protocolo distinto del autorizado.

| Momento | Qué se realizó, observó o analizó | Comprobación de calidad o seguridad | Evidencia asociada |
| --- | --- | --- | --- |
| Preparación del puesto y consulta del [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/) | [Escribe aquí] | [EPI, orden, residuos] | [Foto 1, nota u otra] |
| Identificación del inóculo y medio | [Escribe aquí] | [Código, lote, estandarización] | [Escribe aquí] |
| Siembra, demostración o simulación | [Escribe aquí] | [Uniformidad / supervisión] | [Foto 2, nota u otra] |
| Identificación y colocación de discos | [Escribe aquí] | [Código, carga, distribución autorizada] | [Escribe aquí] |
| Custodia hasta la lectura | [Escribe aquí] | Según [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/) | [Escribe aquí] |
| Medida y lectura de halos | [Escribe aquí] | [Instrumento, unidad, observaciones] | [Foto 3, tabla u otra] |
| Consulta del estándar e interpretación | [Escribe aquí] | [Versión y aplicabilidad] | [Escribe aquí] |
| Gestión final de residuos y puesto | [Escribe aquí] | [Ruta indicada] | [Escribe aquí] |

---

## 9. Resultados y controles

### [ALUMNADO · RELLENABLE] Comprobación de controles

| Elemento que se revisa | Resultado | Evidencia | Consecuencia para la interpretación |
| --- | --- | --- | --- |
| Modalidad y trazabilidad correctamente declaradas | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Inóculo estandarizado o documentado | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Medio adecuado e íntegro | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Discos identificados, vigentes y bien conservados | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Siembra uniforme y distribución legible | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Control de calidad dentro de lo esperado | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |
| Estándar vigente y aplicable disponible | [Conforme / no conforme / no aplica] | [Escribe aquí] | [Escribe aquí] |

### [ALUMNADO · RELLENABLE] Medida de halos

| Disco / antimicrobiano | Código, carga o identificación | Diámetro del halo (mm) | Forma y borde | Observaciones o incidencias | Fuente de evidencia |
| --- | --- | --- | --- | --- | --- |
| 1 | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Propia / docente / simulación / datos] |
| 2 | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Propia / docente / simulación / datos] |
| 3 | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Propia / docente / simulación / datos] |
| 4 | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Propia / docente / simulación / datos] |
| 5 | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Propia / docente / simulación / datos] |
| 6 | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Escribe aquí] | [Propia / docente / simulación / datos] |

### [ALUMNADO · RELLENABLE] Interpretación según estándar vigente

| Disco / antimicrobiano | Diámetro medido | Tabla o punto de corte consultado | Categoría resultante | ¿Aplicable y válida? | Comentario |
| --- | --- | --- | --- | --- | --- |
| 1 | [mm] | [Referencia] | [Categoría / no interpretable] | [Sí / no / dudosa] | [Escribe aquí] |
| 2 | [mm] | [Referencia] | [Categoría / no interpretable] | [Sí / no / dudosa] | [Escribe aquí] |
| 3 | [mm] | [Referencia] | [Categoría / no interpretable] | [Sí / no / dudosa] | [Escribe aquí] |
| 4 | [mm] | [Referencia] | [Categoría / no interpretable] | [Sí / no / dudosa] | [Escribe aquí] |
| 5 | [mm] | [Referencia] | [Categoría / no interpretable] | [Sí / no / dudosa] | [Escribe aquí] |
| 6 | [mm] | [Referencia] | [Categoría / no interpretable] | [Sí / no / dudosa] | [Escribe aquí] |

### [ALUMNADO · RELLENABLE] Control de anomalías

| Observación | ¿Se observó? | Posible efecto | Acción o limitación registrada |
| --- | --- | --- | --- |
| Crecimiento no uniforme | [Sí / no / no valorable] | [Escribe aquí] | [Escribe aquí] |
| Colonias dentro del halo | [Sí / no / no valorable] | [Escribe aquí] | [Escribe aquí] |
| Halo irregular o doble | [Sí / no / no valorable] | [Escribe aquí] | [Escribe aquí] |
| Halos solapados | [Sí / no / no valorable] | [Escribe aquí] | [Escribe aquí] |
| Contaminación o cultivo mixto | [Sí / no / no valorable] | [Escribe aquí] | [Escribe aquí] |
| Disco desplazado, deteriorado o sin identificar | [Sí / no / no valorable] | [Escribe aquí] | [Escribe aquí] |

### [ALUMNADO · RELLENABLE] Síntesis del resultado

> Resume en 3–5 líneas los diámetros obtenidos, la validez del ensayo, las categorías asignadas y la principal limitación o necesidad de repetición.

[Escribe aquí]

---

## 10. Evidencias visuales

### [ALUMNADO · RELLENABLE]

Incluye imágenes propias solo si su captura está autorizada. Si proceden del centro, de una simulación o de una placa preparada, indícalo expresamente. Evita mostrar rostros, datos personales, códigos sensibles o resultados de muestras no autorizadas.

### Imagen 1 — Preparación e identificación

![Preparación e identificación](../assets/P18/preparacion_e_identificacion_01.jpg)

> **Indicaciones:** Puede mostrar el puesto preparado, el medio, el patrón de inóculo o los discos identificados sin datos sensibles.

- Pie de foto técnico: [Escribe aquí]
- Autoría y procedencia: [Propia / docente / simulación / material de referencia]
- Qué demuestra: [Escribe aquí]

### Imagen 2 — Siembra o distribución de discos

![Siembra o distribución de discos](../assets/P18/siembra_o_distribucion_de_discos_02.jpg)

> **Indicaciones:** Puede mostrar una demostración autorizada, una simulación o la placa antes de la lectura, siempre que su publicación esté permitida.

- Pie de foto técnico: [Escribe aquí]
- Autoría y procedencia: [Propia / docente / simulación / material de referencia]
- Qué demuestra: [Escribe aquí]

### Imagen 3 — Lectura de halos

![Lectura de halos](../assets/P18/lectura_de_halos_03.jpg)

> **Indicaciones:** Debe permitir reconocer la placa o el resultado docente y asociar los halos a los discos registrados, sin revelar información sensible.

- Pie de foto técnico: [Escribe aquí]
- Autoría y procedencia: [Propia / docente / simulación / material de referencia]
- Qué demuestra y qué no permite concluir: [Escribe aquí]

---

## 11. Incidencias y acciones correctoras

### [ALUMNADO · RELLENABLE]

| Incidencia, duda o desviación | Riesgo o efecto posible | Acción adoptada | Comunicación al centro | Impacto en el resultado |
| --- | --- | --- | --- | --- |
| [Escribe aquí o “No se registraron incidencias”] | [Escribe aquí] | [Escribe aquí] | [Sí / no / no aplica] | [Escribe aquí] |

---

## 12. Interpretación técnica

### [ALUMNADO · RELLENABLE]

Redacta una interpretación breve y fundamentada:

1. Explica si el inóculo, el medio, los discos, la distribución y los controles permiten considerar válido el ensayo.
2. Presenta los diámetros como datos brutos y describe cualquier anomalía de la lectura.
3. Asigna las categorías utilizando el estándar vigente y cita organismo, versión y fecha de la tabla consultada.
4. Indica qué resultados son dudosos, no aplicables o requieren repetición.
5. Separa la interpretación docente de cualquier decisión clínica, que queda fuera del alcance de esta práctica.

[Escribe aquí]

> **Recordatorio.** Un halo mayor no significa automáticamente una mejor opción terapéutica. La categoría depende del antimicrobiano, el organismo, el método y los puntos de corte vigentes.

---

## 13. Conclusión

### [ALUMNADO · RELLENABLE]

> Redacta una conclusión de 4–6 líneas. Debe diferenciar entre: (a) la modalidad realmente desarrollada; (b) la validez y los diámetros registrados; y (c) la interpretación profesional obtenida y sus límites.

[Escribe aquí]

---

## 14. Reflexión profesional

### [ALUMNADO · RELLENABLE]

1. **Procedimiento.** ¿Qué factor del inóculo, del medio, de los discos o de la distribución consideras más crítico para obtener halos comparables? Justifica tu elección.

   [Respuesta]

2. **Interpretación.** ¿Por qué es necesario conservar el diámetro medido y consultar después el estándar vigente, en lugar de interpretar el halo solo por su tamaño visual?

   [Respuesta]

3. **Conclusiones.** ¿Qué puedes afirmar sobre la respuesta de la cepa docente frente a los antimicrobianos ensayados y qué no puedes afirmar sobre un tratamiento para una persona?

   [Respuesta]

4. **Aprendizaje y transferencia.** ¿Cómo aplicarías la trazabilidad, el control de calidad y la comunicación de resultados dudosos en otra prueba de sensibilidad o identificación?

   [Respuesta]

---

## 15. Trazabilidad y entrega

### [ALUMNADO · RELLENABLE]

| Elemento | Registro |
| --- | --- |
| Código de práctica | P18 |
| Fecha de entrega | [dd/mm/aaaa] |
| Enlace o ubicación de las evidencias | [Escribe aquí] |
| Modalidad declarada | [Escribe aquí] |
| [PNT](https://www.eucast.org/bacteria/methodology-and-instructions/disk-diffusion-and-quality-control/), estándar profesional y tabla consultada | [Escribe aquí] |
| Cepa/evidencia, inóculo, medio y discos | [Escribe aquí] |
| Diámetros brutos y categorías resultantes | [Escribe aquí] |
| Incidencias, repetición o no interpretación | [Escribe aquí] |
| Ruta de residuos o cierre de actividad | [Escribe aquí] |
| Autoevaluación | [Logrado / en proceso / necesito revisión] |

---
